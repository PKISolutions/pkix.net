...
public sealed class X509FreshestCRLExtension : X509Extension {
    static readonly Oid _oid = new(X509ExtensionOid.FreshestCRL);
    readonly List<X509DistributionPoint> _distPoints = new();

    X509FreshestCRLExtension(Byte[] rawData, Boolean critical) : base(_oid, rawData, critical) {
        if (rawData == null) {
    ...
    /// </summary>
    public X509DistributionPoint[] FreshestCrlDistributionPoints => _distPoints.ToArray();

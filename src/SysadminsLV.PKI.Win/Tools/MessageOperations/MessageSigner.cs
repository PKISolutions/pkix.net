...
/// </remarks>
public class MessageSigner : ICryptSigner, IDisposable {
    ...
    void getSignatureAlgorithm() {
        switch (keyType) {
            case KeyType.EcDsa:
                ...
                break;
            case KeyType.Rsa:
                SignatureAlgorithm = PaddingScheme == RSASignaturePadding.Pss
    ...
    Byte[] signHashCngGeneric<T>(Byte[] hash, T padding, NCryptSignHash<T> signer)
        where T : struct {
        SignaturePadding rawPadding = PaddingScheme == RSASignaturePadding.Pkcs1 ? SignaturePadding.PKCS1 : SignaturePadding.PSS;
    ...
    Boolean verifyHashRsa(Byte[] hash, Byte[] signature) {
        return PaddingScheme == RSASignaturePadding.Pss
        ...
        switch (PublicKeyAlgorithm.Value) {
            case AlgorithmOid.ECC: // ECDSA
                if (alternate) {
                    ...
                    parameters
                        .AddRange(
                            new AlgorithmIdentifier(HashingAlgorithm, new Asn1Null().GetRawData()).RawData
                        );
                }
                break;
